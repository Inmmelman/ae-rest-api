...
class VoucherAttachToProductTest extends TestCase
{
	...
	public function is_return_correct_message_on_attach_voucher_to_product()
	{
		...
		$response = $this->putJson('/api/products/' . $prod->id . '/vouchers/' . $voucher->id);
		$response->assertStatus(200)
			->assertJson(
				[
					"message" => "Voucher with id 1 has been attached to product with id 1"
				]
			);
	}
}
